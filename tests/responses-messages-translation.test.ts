...
describe("Responses Lite to Messages translation", () => {
  ...
  test("drops web search tools instead of rejecting the request", () => {
    // Codex advertises web_search whenever it lacks metadata for a model; the
    // Messages fallback cannot run it, but the request must still succeed.
    const result = translate({
      input: "hello",
      tools: [
        { type: "web_search" },
