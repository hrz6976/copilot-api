...
function registerTool(
  ...
  // Same policy as resolveWebSearchRoute on the Messages path: a web_search
  // mixed with other tools is dropped rather than routed to
  // messageApiWebSearchModel (only web-search-only requests are switched).
  // Codex always sends web_search alongside its shell/apply_patch tools, so
  // rejecting here would make every Messages-fallback model unusable from it.
